...
function get_leenkme_expanded_fb_post( $post_id, $facebook_array, $post_title = false, $excerpt = false ) {
	...
	if ( !empty( $facebook_array ) ) {
		...
		$post_title = htmlspecialchars( stripcslashes( $post_title ) );
		...
		if ( false === $excerpt ) {
			...
			if ( !empty( $post->post_excerpt ) ) {
				...
				$excerpt = htmlspecialchars( stripcslashes( $post->post_excerpt ) ); 
				
			} else {
				
				//otherwise we'll pare down the description
				$excerpt = htmlspecialchars( stripcslashes( $post->post_content ) ); 
