		...
		function print_facebook_settings_page() {
			...
			if ( isset( $_POST['update_facebook_settings'] ) ) {
				...
				if ( isset( $_POST['facebook_message'] ) )
					$user_settings['facebook_message'] = $_POST['facebook_message'];
	
				if ( isset( $_POST['facebook_linkname'] ) )
					$user_settings['facebook_linkname'] = $_POST['facebook_linkname'];
				
				if ( isset( $_POST['facebook_caption'] ) )
					$user_settings['facebook_caption'] = $_POST['facebook_caption'];
				
				if ( isset( $_POST['facebook_description'] ) )
					$user_settings['facebook_description'] = $_POST['facebook_description'];
				
				if ( isset( $_POST['default_image'] ) )
					$user_settings['default_image'] = $_POST['default_image'];
				
				if ( isset( $_POST['force_facebook_image'] ) )
				...
				if ( isset( $_POST['clude'] ) && isset( $_POST['publish_cats'] ) ) {
					...
					$user_settings['publish_cats'] = $_POST['publish_cats'];
				...
				<form id="leenkme" method="post" action="">
					...
					<div id="post-types" class="postbox">
						...
						<div class="inside">
                            ...
                            <tr>
                            	<td><?php _e( 'Default Message:', 'leenkme' ); ?></td>
...
function get_leenkme_expanded_fb_post( $post_id, $facebook_array, $post_title = false, $excerpt = false ) {
	...
	if ( !empty( $facebook_array ) ) {
		...
		$facebook_array['description'] 	= leenkme_trim_words( leenkme_replacements_args( $facebook_array['description'], $post_title, $excerpt, true ), $maxDescLen );
...
function get_leenkme_expanded_fb_post_ajax() {
	...
	if ( isset( $_POST['facebook_array'] ) )
		$facebook_array = $_POST['facebook_array'];
	else
	...
	die( json_encode( get_leenkme_expanded_fb_post( $post_id, $facebook_array, $title, $excerpt ) ) );
...
function leenkme_ajax_republish() {
	...
	if ( isset( $_POST['id'] ) && isset( $_POST['facebook_array'] ) ) {
		...
		if ( get_post_meta( $_POST['id'], 'facebook_exclude_profile', true ) 
			...
			$results = leenkme_ajax_connect( leenkme_publish_to_facebook( array(), $_POST['id'], $_POST['facebook_array'], true ) );
				...
				if ( !empty( $options ) ) {
					...
					if ( empty( $facebook_array ) ) {
						...
						if (   !( $facebook_array['message'] 		= get_post_meta( $post_id, '_facebook_message', true ) ) 
							&& !( $facebook_array['linkname'] 		= get_post_meta( $post_id, '_facebook_linkname', true ) ) 
							&& !( $facebook_array['caption'] 		= get_post_meta( $post_id, '_facebook_caption', true )  ) 
							&& !( $facebook_array['description']	= get_post_meta( $post_id, '_facebook_description', true )  ) 
							&& !( $facebook_array['picture']		= get_post_meta( $post_id, '_facebook_image', true ) ) ) {
...
if ( isset( $dl_pluginleenkmeFacebook ) ) {
	...
	add_action( 'wp_ajax_get_leenkme_expanded_fb_post', 'get_leenkme_expanded_fb_post_ajax' );
