	...
	class leenkme_FriendFeed {
		...
		function print_friendfeed_settings_page() {
			...
			if ( isset( $_POST['update_friendfeed_settings'] ) ) {
				...
				if ( isset( $_POST['friendfeed_body'] ) )
					$user_settings['friendfeed_body'] = $_POST['friendfeed_body'];
				
				if ( isset( $_POST['default_image'] ) )
					$user_settings['default_image'] = $_POST['default_image'];
				
				if ( isset( $_POST['force_friendfeed_image'] ) )
				...
				if ( isset( $_POST['clude'] ) && isset( $_POST['feed_cats'] ) ) {
					...
					$user_settings['feed_cats'] = $_POST['feed_cats'];
			...
			<div class="meta-box-sortables ui-sortable">
				<form id="leenkme" method="post" action="">
					<h2 style='margin-bottom: 10px;' ><img src='<?php echo $dl_pluginleenkme->base_url; ?>/leenkme-logo-32x32.png' style='vertical-align: top;' /> FriendFeed <?php _e( 'Settings', 'leenkme' ); ?> (<a href="http://leenk.me/2011/04/08/how-to-use-the-leenk-me-friendfeed-plugin-for-wordpress/" target="_blank">help</a>)</h2>
					...
					<div id="post-types" class="postbox">
						...
						<div class="inside">
                            ...
                            <tr>
                            	<td><?php _e( 'Default Description:', 'leenkme' ); ?></td>
		...
		function leenkme_friendfeed_meta_tags( $post_id ) {
			...
			if ( isset( $_POST["friendfeed_exclude_myfeed"] ) )
				update_post_meta( $post_id, '_friendfeed_exclude_myfeed', $_POST["friendfeed_exclude_myfeed"] );
			else
				delete_post_meta( $post_id, '_friendfeed_exclude_myfeed' );
	
			if ( isset( $_POST["friendfeed_exclude_group"] ) )
				update_post_meta( $post_id, '_friendfeed_exclude_group', $_POST["friendfeed_exclude_group"] );
			else
				delete_post_meta( $post_id, '_friendfeed_exclude_group' );
			
			if ( isset( $_POST['friendfeed_body'] ) && !empty( $_POST['friendfeed_body'] ) )
				update_post_meta( $post_id, '_friendfeed_body', $_POST['friendfeed_body'] );
			else
				delete_post_meta( $post_id, '_friendfeed_body' );
	
			if ( isset($_POST["friendfeed_image"] ) && !empty( $_POST["friendfeed_image"] ) )
				update_post_meta( $post_id, '_friendfeed_image', $_POST["friendfeed_image"] );
			else
				delete_post_meta( $post_id, '_friendfeed_image' );
	
			if ( isset($_POST["lm_friendfeed_type"] ) && !empty( $_POST["lm_friendfeed_type"] ) )
				update_post_meta( $post_id, '_lm_friendfeed_type', $_POST["lm_friendfeed_type"] );
			else
				delete_post_meta( $post_id, '_lm_friendfeed_type' );
...
function get_leenkme_expanded_ff_post( $post_id, $friendfeed_array, $post_title = false, $excerpt = false ) {
	...
	if ( !empty( $friendfeed_array ) ) {
		...
		$friendfeed_array['body'] = leenkme_trim_words( leenkme_replacements_args( $friendfeed_array['body'], $post_title, $excerpt, true ), $maxBodyLen );
...
function get_leenkme_expanded_ff_post_ajax() {
	...
	if ( isset( $_POST['friendfeed_array'] ) )
		$friendfeed_array = $_POST['friendfeed_array'];
	else
	...
	die( json_encode( get_leenkme_expanded_ff_post( $post_id, $friendfeed_array, $title, $excerpt ) ) );
...
function leenkme_ajax_refeed() {
	...
	if ( isset( $_POST['id'] ) && isset( $_POST['friendfeed_array'] ) ) {
		...
		if ( get_post_meta( $_POST['id'], 'friendfeed_exclude_myfeed', true ) 
			...
			$results = leenkme_ajax_connect( leenkme_publish_to_friendfeed( array(), $_POST['id'], $_POST['friendfeed_array'], true ) );
				...
				if ( !empty( $options ) ) {
					...
					if ( empty( $friendfeed_array ) ) {
						...
						if ( 	!( $friendfeed_array['body'] 	= get_post_meta( $post_id, '_friendfeed_body', true ) )
								&& !( $friendfeed_array['picture'] = get_post_meta( $post_id, '_friendfeed_image', true ) ) ) {
					...
					$connect_arr[$api_key]['friendfeed_body'] = $friendfeed_array['body'];
					$connect_arr[$api_key]['friendfeed_link'] = $url;
...
if ( isset( $dl_pluginleenkmeFriendFeed ) ) {
	...
	add_action( 'wp_ajax_get_leenkme_expanded_ff_post', 'get_leenkme_expanded_ff_post_ajax' );
