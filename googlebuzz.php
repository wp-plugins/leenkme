	...
	class leenkme_GoogleBuzz {	 
		...
		function print_googlebuzz_settings_page() {
			...
			<div class="meta-box-sortables ui-sortable">
				<form id="leenkme" method="post" action="">
					<h2 style='margin-bottom: 10px;' ><img src='<?php echo $dl_pluginleenkme->base_url; ?>/leenkme-logo-32x32.png' style='vertical-align: top;' /> Google Buzz Settings (<a href="http://leenk.me/2010/09/04/how-to-use-the-leenk-me-google-buzz-plugin-for-wordpress/" target="_blank">help</a>)</h2>
					...
					<div id="post-types" class="postbox">
                        ...
                        <p>
                            <input type="button" class="button" name="verify_googlebuzz_connect" id="gb_buzz" value="<?php _e( 'Buzz a Test Message', 'leenkme_GoogleBuzz' ) ?>" />
                            <?php wp_nonce_field( 'gb_buzz', 'gb_buzz_wpnonce' ); ?>
		...
		function leenkme_add_googlebuzz_meta_tag_options() { 
			...
			foreach ( $leenkme_settings['post_types'] as $post_type ) {
				...
				add_meta_box( 
					'leenkme-GoogleBuzz',
					__( 'leenk.me Google Buzz', 'leenkme' ),
					array( $this, 'leenkme_googlebuzz_meta_box' ),
					$post_type 
				);
		...
		function leenkme_googlebuzz_meta_box() {
			...
			<table>			
				...
				<td>
					<input style="margin-top: 5px;" type="checkbox" name="googlebuzz_exclude" <?php checked( $exclude || 'on' == $exclude ); ?> />
				</td></tr>
...
if ( isset( $dl_pluginleenkmeGoogleBuzz ) ) {
	...
	add_action( 'admin_init', array( $dl_pluginleenkmeGoogleBuzz, 'leenkme_add_googlebuzz_meta_tag_options' ), 1 );
